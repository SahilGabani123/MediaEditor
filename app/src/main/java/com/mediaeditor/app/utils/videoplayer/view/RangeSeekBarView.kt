package com.mediaeditor.app.utils.videoplayer.view
...
import com.mediaeditor.app.utils.videoplayer.event.OnRangeSeekBarEvent
...
class RangeSeekBarView @JvmOverloads constructor(
    ...
    private fun init() {
        ...
        mBorder.strokeWidth = N_15f // Set border width as needed
    }
    ...
    private fun drawBorder(canvas: Canvas) {
        if (thumbs.size == 2) {
            ...
            val margin = N_18f // Margin for the start and end
            ...
            val cornerRadius = N_15f // Radius for rounded corners
    ...
    override fun onTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (currentThumb == 0) {
                    when {
                        newX + mThumb.widthBitmap >= mThumb2.pos ->
                            mThumb.pos =
    ...
    private fun pixelToScale(index: Int, pixelValue: Float): Float {
        val scale = pixelValue * N_100 / mPixelRangeMax
        return if (index == 0) {
            val pxThumb = scale * mThumbWidth / N_100
            scale + pxThumb * N_100 / mPixelRangeMax
        } else {
            val pxThumb = (N_100 - scale) * mThumbWidth / N_100
            scale - pxThumb * N_100 / mPixelRangeMax
        }
    }

    private fun scaleToPixel(index: Int, scaleValue: Float): Float {
        val px = scaleValue * mPixelRangeMax / N_100
        return if (index == 0) {
            val pxThumb = scaleValue * mThumbWidth / N_100
            px - pxThumb
        } else {
            val pxThumb = (N_100 - scaleValue) * mThumbWidth / N_100
            px + pxThumb
        }
    }
    ...
    private fun drawShadow(canvas: Canvas) {
        if (thumbs.isNotEmpty()) {
            for (th in thumbs) {
                if (th.index == 0) {
                    drawLeftShadow(canvas, th)
                } else {
                    drawRightShadow(canvas, th)
                }
            }
        }
    }

    private fun drawRightShadow(canvas: Canvas, th: Thumb) {
    ...
    private fun drawLeftShadow(canvas: Canvas, th: Thumb) {
